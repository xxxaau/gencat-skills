# Identitat Visual — Marca Corporativa Generalitat de Catalunya

Font canònica per a colors i tipografia. Tots els altres fitxers de referència d'aquest skill remeten aquí.

Font oficial: https://identitatcorporativa.gencat.cat/ca/identitat-visual/ (citar en respostes de consulta i informes de validació)

---

## Colors corporatius

| Color | Pantone | CMYK | RGB | Hex | RAL |
|-------|---------|------|-----|-----|-----|
| **Vermell (principal)** | 485 | C:0 M:100 Y:91 K:0 | R:192 G:0 B:0 | #C00000 | 3020 |
| Negre | Process Black | C:0 M:0 Y:0 K:100 | R:0 G:0 B:0 | #000000 | — |
| Blanc | — | C:0 M:0 Y:0 K:0 | R:255 G:255 B:255 | #FFFFFF | — |
| Groc *(ús específic)* | 109 | — | — | #FFD100 | — |
| Or *(ús específic)* | 871 | — | — | #857040 | — |

**Regles d'ús:**
- El **vermell** (#C00000) és l'únic color corporatiu principal
- **Blanc** i **negre** proporcionen contrast base
- **Groc** i **or** s'usen exclusivament en aplicacions molt específiques; no en documents estàndard
- Cap altre color és permès com a color dominant en materials corporatius

---

## Tipografia per suport

| Suport | Tipografia | Notes |
|--------|-----------|-------|
| Corporativa (logotip) | Helvetica Neue 75 Bold / 45 Light | Llicència de pagament; no usar en documents Office sense llicència |
| Documents Office (Word, PowerPoint, formularis) | Arial / Arial Bold | Gratuïta; substitut oficial de Helvetica Neue |
| Digital (web, newsletters, infografies, vídeo) | Open Sans | Preferida per legibilitat en pantalla |
| Signatura de correu electrònic | Arial | **Excepció tècnica:** les signatures es renderitzen al client del receptor, que no té accés a fonts web. Open Sans **no** s'ha d'usar en signatures de correu |
| ❌ Prohibida | Calibri | No és tipografia corporativa |

---

## Normes del senyal corporatiu

- **Mida mínima:** 4,7 mm d'alçada
- **Colors permesos:** vermell (#C00000) o negre (#000000)
- **Versió 3 colors** (vermell + negre + daurat): únicament en aplicacions molt específiques
- **Espai de seguretat:** no superposar text ni elements visuals sobre el senyal
- **Prohibit:** distorsionar, estirar o comprimir; canviar les proporcions originals
- **Reversible:** el disseny és simètric per evitar errors de reproducció

---

## Normes del logotip

- **Tipografia exclusiva:** Helvetica Neue 75 Bold (un sol nivell jeràrquic) o Helvetica Neue 45 Light (dos o més nivells)
- **Cap altra font** pot substituir Helvetica Neue al logotip (ni Arial)
- **Distribució:** una línia (composicions centrades) o dues línies entre "Generalitat" i "de Catalunya"
- **Escriptura correcta:** "Generalitat de Catalunya" (G i C en majúscula, la resta en minúscula)
- **Regla clau:** en comunicacions adreçades a la ciutadania, usar el logotip de la Generalitat, **NO el departamental**
